...
#[task]
pub async fn send_mqtt_message(stack: &'static Stack<WifiDevice<'static,WifiStaDevice>>, receiver: Receiver<'static, NoopRawMutex,Message,5>, sender: Sender<'static, NoopRawMutex,Message,5>)->! {
    let mut counter = 0;
    loop {
        if !stack.is_link_up() {
        ...
        let mut mqtt_client_config: ClientConfig<'_, 5, CountingRng> = ClientConfig::new(MqttVersion::MQTTv5, CountingRng(12345));
        ...
        mqtt_client.subscribe_to_topic("esp32_test_configuration").await.unwrap();
        loop {
            info!("Loop!");
            match select(receiver.receive(), mqtt_client.receive_message()).await {
                Either::First((topic,payload)) => mqtt_client.send_message(&topic, &payload, rust_mqtt::packet::v5::publish_packet::QualityOfService::QoS1, false).await.unwrap(),
