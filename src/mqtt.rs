...
use log::{info, warn};
use rust_mqtt::{client::{client::MqttClient, client_config::{ClientConfig, MqttVersion}}, packet::v5::publish_packet::QualityOfService, utils::rng_generator::CountingRng};

use crate::MessageReceiver;


#[task]
pub async fn send_mqtt(stack: &'static Stack<WifiDevice<'static,WifiStaDevice>>, host: &'static str, message_receiver: MessageReceiver)->! {
    ...
    loop {
        ...
        let config: ClientConfig<'_, 5, _> = ClientConfig::new(MqttVersion::MQTTv5, CountingRng(30000));
        // config.add_client_id("floodplain");
        // config.max_packet_size = 100;
        let mut receive_buffer = [0;BUFFER_SIZE];
        let mut send_buffer = [0;BUFFER_SIZE];
        let mut mqtt_client = MqttClient::new(tcp_connection, &mut send_buffer, BUFFER_SIZE, &mut receive_buffer, BUFFER_SIZE, config);
        mqtt_client.connect_to_broker().await.unwrap();

        loop {
            let (topic,body) = message_receiver.receive().await;
            match mqtt_client.send_message(&topic, body.as_bytes(), QualityOfService::QoS1, false).await {
                Ok(()) => {},
                Err(_) => {
                    warn!("Send failed");
                    break
        ...
        info!("Connected!");
        for _ in 0..10 {
            mqtt_client.send_message("floodplain", format!("some_content: {}",counter).as_bytes(), QualityOfService::QoS1, false).await.unwrap();
            counter+=1;
            info!("Message sent!");
